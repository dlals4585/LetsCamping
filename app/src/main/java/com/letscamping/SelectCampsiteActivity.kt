...
class SelectCampsiteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val model : SelectCampsiteViewModel by viewModels()
//      ex)  model.getUsers().observe(this, observe(this,Observer<List<User>>{
//      })
        ...
        model.getGoCamplist_keyword(keyword)

        if(keyword1 != ""){
            binding.textview1.text = "$keyword/$keyword1"
            model.getGoCamplist_keyword(keyword1)
        }

        val selectListadapter = SelectListViewAdapter(applicationContext, model.camplist)
    ...
    class SelectCampsiteViewModel : ViewModel(){
        var camplist : ArrayList<CampingList> = ArrayList<CampingList>()
